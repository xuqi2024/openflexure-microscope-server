...
class rom_methods(Thing):
    def estimated_centre(self, data:dict) -> float:
        '''
        Calculates the centre position of each axis and returns coordinates.
        '''
        x_middle = (data['rom_data']['x']['1']['final_position']['x'] + data['rom_data']['x']['-1']['final_position']['x'])/2
        y_middle = (data['rom_data']['y']['1']['final_position']['y'] + data['rom_data']['y']['-1']['final_position']['y'])/2
        return [x_middle, y_middle]

    def calculate_rom(self, data:dict) -> float:
        '''
        Calculates the range of motion.
        '''
        rom_x_steps = (data['rom_data']['x']['1']['final_position']['x'] - data['rom_data']['x']['-1']['final_position']['x'])
        rom_y_steps = (data['rom_data']['y']['1']['final_position']['y'] - data['rom_data']['y']['-1']['final_position']['y'])
    ...
    def get_stage_positions(self, data:dict, axis:str, key:str):
        '''
        Extracts a single coordinate variable (x, y or z) from each stage coordinate in a dataset.
        '''
        return [pos[key] for d in ['1', '-1'] for pos in data['rom_data'][axis][d]['stage_positions']]

    def find_polarity(self, data:dict, dir:str) -> str:
        '''
        Determines the polarity of the z stepper motor by fitting a curve of the x and z positions along a single axis.
        '''
        x_pos = self.get_stage_positions(data, dir, dir)
        z_pos = self.get_stage_positions(data, dir, 'z')
        params, _ = curve_fit(quadratic, x_pos, z_pos)
        coef = 2 * params[0]
        curvature = 'positive' if coef > 0 else 'negative' if coef < 0 else 'ERROR'
        return curvature, params, x_pos, z_pos

    def extract_differences(self, data:dict) -> dict:
        '''
        ...
        for axis in ['x', 'y']:
            ...
            for d in ['1', '-1']:
                sp = data['rom_data'][axis][d]['stage_positions']
    ...
    def extract_positions(self, data:dict) -> dict:
        '''
    ...
    def extract_correlations(self, data:dict) -> dict:
        '''
        ...
        for axis in ['x', 'y']:
            ...
            for d in ['1', '-1']:
                cor_dic[axis][d] = [corr[axis_idx] for corr in data['rom_data'][axis][d]['correlation_lateral_steps']]
        return cor_dic

    def flatten_dict(self, dic:str, axis:str, trim:bool=False):
        '''
    ...
    def csm_analysis(self, stage_dic:dict, pos_dic:dict, cor_dic:dict) -> dict:
        '''
        Completes the csm analysis. First creates all the necessary arrays and returns parameters from
        curve fit.
        '''
        x_stage = self.flatten_dict(stage_dic, 'x', trim=False)
        y_stage = self.flatten_dict(stage_dic, 'y', trim=False)
        x_stage_fit = self.flatten_dict(stage_dic, 'x', trim=True)
        y_stage_fit = self.flatten_dict(stage_dic, 'y', trim=True)
        x_stage_pos = self.flatten_dict(pos_dic, 'x', trim=False)
        y_stage_pos = self.flatten_dict(pos_dic, 'y', trim=False)
        x_pos_fit = self.flatten_dict(pos_dic, 'x', trim=True)
        y_pos_fit = self.flatten_dict(pos_dic, 'y', trim=True)
        x_cor = self.flatten_dict(cor_dic, 'x', trim=False)
        y_cor = self.flatten_dict(cor_dic, 'y', trim=False)
        x_cor_fit = self.flatten_dict(cor_dic, 'x', trim=True)
        y_cor_fit = self.flatten_dict(cor_dic, 'y', trim=True)
        ...
        params_x, _ = curve_fit(straight_line, x_pos_fit, np.abs(x_pixel_step_fit))
        params_y, _ = curve_fit(straight_line, y_pos_fit, np.abs(y_pixel_step_fit))

        xaxis_coord = np.arange(np.min(x_pos_fit), np.max(x_pos_fit), 1)
        xaxis_pixel_step = straight_line(xaxis_coord, params_x[0], params_x[1])

        yaxis_coord = np.arange(np.min(y_pos_fit), np.max(y_pos_fit), 1)
        yaxis_pixel_step = straight_line(yaxis_coord, params_y[0], params_y[1])

        return {
        'x_pixel_step': x_pixel_step,
        'y_pixel_step': y_pixel_step,
        ...
        'params_x': params_x,
        'params_y': params_y
        }, x_stage_pos, y_stage_pos, xaxis_coord, xaxis_pixel_step, yaxis_coord, yaxis_pixel_step    

    def compute_rom_error(self, cor_dic:dict) -> float:
        '''
    ...
    def rom_analysis(self) -> json:
        '''
        Completes all analysis steps for range of motion and returns a json of all the information.
        '''
        ...
        est_centre = self.estimated_centre(data)
        rom = self.calculate_rom(data)
        curvature, params, x_pos, z_pos = self.find_polarity(data, 'x')
        stage_dic = self.extract_differences(data)
        pos_dic = self.extract_positions(data)
        cor_dic = self.extract_correlations(data)
        csm_dic, x_stage_pos, y_stage_pos, xaxis_coord, xaxis_pixel_step, yaxis_coord, yaxis_pixel_step = self.csm_analysis(stage_dic, pos_dic, cor_dic)
        rom_err_x, rom_err_y = self.compute_rom_error(cor_dic)
        ...
        if np.max(np.abs(x_stage_pos)) > np.min(np.abs(x_stage_pos)):
            xlim = np.max(np.abs(x_stage_pos)) + 5000
        else:
            xlim = np.min(np.abs(x_stage_pos)) + 5000

        plot_function("CSM across ROM", "Stage Coordinate", "Pixel/Step")
        plt.scatter(x_stage_pos, np.abs(csm_dic['x_pixel_step']), label = 'X axis', color = '#C5247F')
        plt.scatter(y_stage_pos, np.abs(csm_dic['y_pixel_step']), label = 'Y axis', color = '#24c5bb')
        ...
        plt.plot(yaxis_coord, yaxis_pixel_step, label = f"Y Axis Best Fit, m = {np.format_float_scientific(csm_dic['params_y'][0], 2)}", color = 'red', linewidth = 2)
        plt.xlim(-xlim, xlim)
        plt.legend()
        ...
        ROM_lim_x = abs(rom[0][0]) - 2
        ROM_lim_y = abs(rom[0][1]) - 2
    ...
    def calibration_data_generate(self):
        ...
        data_txt = (f"CSM Matrix:[[1,0],[0,1]]\n"
            f"Pixel/Step:{csm_con}\n"
            f"Range of Motion(Steps):{rom_dict['Range of Motion'][1][0]} X {rom_dict['Range of Motion'][1][1]}\n"
            f"Range of Motion(mm):{rom_dict['Range of Motion'][0][0]} X {rom_dict['Range of Motion'][0][1]}"
            )
