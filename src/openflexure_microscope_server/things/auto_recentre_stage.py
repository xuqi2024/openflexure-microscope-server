...
#InvocationLogger.info(f"{stream_resolution}")
...
def dict_generate(dict_steps, stream_resolution, factor = 1):
    '''
    Creates a single dictionary of step sizes

    Input:
        dict_name(string): Name of the dictionary
        dict_steps(int): Percentage of the field of view covered in one movement
        factor(float): If a minimum number of steps is being created, this is required. Default is 1.

    Returns:
        A single dictionary with the name given by dict_name and 'x' and 'y' step sizes.  
    '''
    dict = {
        'x':(dict_steps/100) * stream_resolution[0] * factor,
        'y':(dict_steps/100) * stream_resolution[1] * factor
    }
    return dict

def steps_generate(small_step, z_perc, big_step, stream_resolution):
    '''
    ...
    '''
    step_sizes_big = dict_generate(big_step, stream_resolution)
    step_sizes_small = dict_generate(small_step, stream_resolution)
    minimum_offset_small = dict_generate(small_step, stream_resolution, factor = 0.8)
    z_steps = dict_generate(z_perc, stream_resolution)
    minimum_offset_z = dict_generate(z_perc, stream_resolution, factor = 0.8)
...
class RangeofMotionThing(Thing):
    ...
    def measure_rom(
        ...
        # defined within the function since it depends on cam, csm and logger
        def motion(step_size, focus_range, minimum_offset, autofocus_proc, focus_data):
            ...
            Input: 
                step size(dict): Number of steps taken between correlations. Usually given in terms of % FOV. 
                                Should be names ['x'] and ['y']. Generated by dict_generate/steps_generate funtion.
            ...
            '''
            # image1 is a captured image, converted to a numpy array with the option of resizing to improve speed.
            ...
            if autofocus_proc == True:
                ...
                # focus_data[1] is the array of all the file sizes so the largest file size is the sharpest image (most 'in focus').
        ...
        try:
            ...
            # base_sharp is a local variable because it is dependant on the sample being used to complete the range of motion test
            base_sharp = np.max(focus_data[1]) - 5000 
            
            # Generates all the required dictionaries for all the different step sizes it will need to complete the ROM test
            step_sizes_small, minimum_offset_small, z_steps, minimum_offset_z, step_sizes_big = steps_generate(20, 50, 200, stream_resolution=stream_resolution)
            ...
            for axs in ['x','y']:
                ...
                for dir in [1, -1]:
                    ...
                    focus_data = autofocus.looping_autofocus(dz = 1000)
                    # The lowest sharpness value allowed. If below this, autofocus procedure occurs. 
                    # This is the maximum sharpness value of the first autofocus that occurs.
                    ...
                    # Repeats 4 times because it takes data from the first 4 positions to predict the proceeding z positions 
                    for loop in range(4):
                        logger.info(f"Current position is {stage.position}")
                        offset, delta, image1, focus_data = motion(
                            step_size = z_cal_step,
                            focus_range = 800, 
                            minimum_offset = minimum_offset_z, 
                            autofocus_proc = True, 
                    ...
                    while (np.abs(delta[axs]) > minimum_offset_small[axs]
                    and np.abs(z_delta[axs]) > minimum_offset_z[axs]\
                    and axis_error == False):  #loop will continue until pixel distance is less than some value
                        pos = stage.position
                        ...
                        for loop in range(3):
                            focused_positions.append(stage.position)
                            offset, delta, image1, focus_data = motion(
                                step_size = this_small_step_size, 
                                focus_range = 400, 
                                minimum_offset = minimum_offset_small, 
                    ...
                    # Now we move the stage until we detect movement and take that position as final. 
                    # This is to account for the extra motion carried out by the big step.
