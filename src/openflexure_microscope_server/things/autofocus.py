...
import logging
import time
from typing import Annotated, Dict, Mapping, Optional, Sequence
...
class JPEGSharpnessMonitor:
    ...
    def move_data(
        self,
        istart: int,
        istop: Optional[int] = None,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Extract sharpness as a function of (interpolated) z"""
        return self.interpolate_move(
            self.data_dict(),
            istart=istart,
            istop=istop,
        )
    ...
    def interpolate_move(
        data: SharpnessDataArrays,
        istart: int,
        istop: Optional[int] = None,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Interpolate the position data to match the sharpness data"""
        if not isinstance(data, SharpnessDataArrays):
            # This ensures the input data is four numpy arrays
            data = SharpnessDataArrays(**data)
        if istop is None:
            istop = istart + 2
        jpeg_times = data.jpeg_times
        jpeg_sizes = data.jpeg_sizes
        stage_times = data.stage_times[istart:istop]
        stage_zs: np.ndarray = np.array(
            [p["z"] for p in data.stage_positions[istart:istop]]
        )
        try:
...
class AutofocusThing(Thing):
    ...
    def verify_focus_sharpness(
        self, sweep_sizes: list, wrappedcamera: WrappedCamera, threshold: float = 0.95
    ):
        """Check the current image is about as sharp as the autofocus peak
    ...
    def autofocus_report(
        ...
        results = {"overshot": 0, "fraction": [], "total": 0}
        ...
        for i in range(repeats):
            ...
            for starts in range(7):
                ...
                _, heights, sizes = JPEGSharpnessMonitor.interpolate_move(
                    data,
                    istart=starts + offset,
                )
        ...
        if plots:
            ...
            with PdfPages(f"logs/{date_stamp}_{time_stamp}_focus.pdf") as pdf:
                ...
                ax.text(
                    0.1,
                    0.8,
                    f"""Autofocus test was run at {time_stamp} on {date_stamp}.
        ...
        logging.info(
            f"Out of {results['total']} trials, it appears that {results['overshot']} overshot."
        )
        return results
